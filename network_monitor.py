...
class Network_Monitor(app_manager.RyuApp):
	...
	self.mydebug = open("debug_monitor.txt",'w')
	...
	self.mydebug.write("pathselected"+str(pathselected)+"\n")
	self.mydebug.write("pathset"+str(pathset)+"\n")
	self.mydebug.flush()
	f2 = open("_linkinfodict.txt",'r')
        str_linkinfodict = f2.read()
	self.mydebug.write("str_linkinfodict"+str_linkinfodict+"\n")
        self._linkinfodict =eval(str_linkinfodict)
	caltotalthroughput = float(0)
	self.mydebug.write("location 1\n")
	self.mydebug.flush()
	for key in pathselected.keys():
	    path = []
	    self.mydebug.write("location 2\n")
	    path = pathset[int(pathselected[key][0])]
	    self.mydebug.write("location 3\n")
	    self.mydebug.write(str(path)+"\n")
	    i = 0
	    ## --- a. get the maxflowrate over the current pathlist
	    maxflowrate = float(0)
	    self.mydebug.write("location 4\n")
	    while(i<len(path)-2):
                inport = int(self._linkinfodict[path[i],path[i+1]][1])
		self.mydebug.write(str(inport)+"\n")
                outport = int(self._linkinfodict[path[i+1],path[i+2]][0])
	        self.mydebug.write(str(outport)+"\n")
		#self.mydebug.write("self.flow_speed"+str(self.flow_speed)+"\n")
		self.mydebug.write("key[0]"+str(key[0])+"\n")
		self.mydebug.write("key[1]"+str(key[1])+"\n")
		self.mydebug.flush()
                if (inport,key[0],key[1],outport) in self.flow_speed[int(path[i+1])].keys():
		    self.mydebug.write("enter\n")
		    self.mydebug.flush()
                    flowrate = abs(self.flow_speed[int(path[i+1])][(inport,key[0],key[1],outport)][-1])
		    self.mydebug.write("location 5\n")
		    self.mydebug.write(str(flowrate)+"\n")
                    if(flowrate > maxflowrate):
                        maxflowrate = flowrate
		i = i + 1
	    ## --- b. sum this maxflowrate
	    caltotalthroughput = caltotalthroughput + maxflowrate
	    self.mydebug.write("location 6\n")
	    self.mydebug.flush()
	throughput_in_Mbit_per_second = float(caltotalthroughput*8) / (1048576);## bytes/s -> Mbits/s.
	self.mydebug.write("location 7\n")
	self.mydebug.flush()
	...
	self.dbugfile1.write("body  "+str(body)+"\n")
        ...
        for stat in sorted( [flow for flow in body if flow.priority == 1 or  flow.priority == 10 ],
...
##############################################################################################################################################
	...
	for mystat in sorted([flow for flow in body if flow.priority == 10],
 	    ...
 	    self.dbugfile1.write("location 1\n")
            self.dbugfile1.flush()
            mykey = (mystat.match['in_port'], mystat.match.get('ipv4_src'),  mystat.match.get('ipv4_dst'),
	    ...
	    self.dbugfile1.write("location 3\n")
            self.dbugfile1.flush()
	    self.dbugfile1.write("mykey"+str(mykey)+"\n")
            self.dbugfile1.flush()
	    self.dbugfile1.write("myvalue"+str(myvalue)+"\n")
            self.dbugfile1.flush()
