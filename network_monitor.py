...
"""
            if self.stats['flow'] or self.stats['port']:
                ...
                self.show_stat('port', self.stats['port'])
		if(isfirsttimecreate and i ==20):
			self.create_caplinkfile(self.stats['port'])
			isfirsttimecreate = False
			i = 0
		elif(i == 50):
			self.create_caplinkfile(self.stats['port'])			
...
""""
    def create_caplinkfile(self,bodys):
	f1 = open("_linkinfodict.txt",'r')
        str_linkinfodict = f1.read()
        self._linkinfodict =eval(str_linkinfodict)
        f1.close()
...
""""
	...
	if port != ofproto_v1_3.OFPP_LOCAL:
		currentcapacity = abs(self.port_speed[(u, port)][-1]),
                            	 self.port_link[u][port][2],
                            	 self.port_link[u][port][0],
                            	 self.port_link[u][port][1]))
