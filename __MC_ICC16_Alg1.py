...
#########################################################################################################################################
	...
	_timeStamp_alg_should_terminate = _timeStamp_Begin_to_run + self.T;	# because the unit-time of self.T is also second.
			...
			if ((current_ts - last_ts_to_check_timer) >= Length_to_check_timers_experiation ):
				...
				if len(ret_timer_check_result)>0:
					...
					for key,val in ret_timer_check_result.items():
						...
						self.Replace_the_selected_DstMBox_and_Path_for_a_SrcCFlow( Src_CFlow, Dst_MBox_cur, Dst_MBox_new, pathID_old, pathID_new );
...
#########################################################################################################################################
	...
	#self.f1.flush()  
        for key,val in self.PathSet_selected.items():
             ...
             DstMB = key[1]
             listPath = val
             print SrcCF,DstMB,listPath 
	#self.f1.write("33\n")
	#self.f1.flush()  
    ## --- B.1 Stage 0: Initialization.
        self.Initialization();
	self.f1.write("Initialization ok \n")
	self.f1.flush()
      #print "\n"+"test22222222222222"
        for key,val in self.PathSet_selected.items():
             SrcCF = key[0]
	...
	self.f1.flush()
    ### --- B.2 Stage 1: Initialize self.Timers for all pairs.
        self.Set_timer_for_all_CFlows(0.0);
    	self.f1.write("15 \n")
	self.f1.flush()
    ## --- C. Enter into time-slot Count-Down process.
        current_ts = 0.0;
    	...
    	self.f1.write("3 \n")
	self.f1.flush()
        while ( current_ts <= self.T ):
          RESET_Msg = 0;
        
        ##### -- C.1 listen to the event of any timer's expiration.
          if ((current_ts-last_ts_to_check_timer)>=self.STEP_TO_CHECK_TIMER):
            ### --- C.1.0 Update the timer-checking time-slot.
              last_ts_to_check_timer = current_ts;
            ...
            ### --- C.1.1 Check self.Timers, if any timer is out, swap its relavant SrcCFlow's hostMBox and path.
              ...
              if len(ret_timer_check_result)>0:
                ...
                # print 'ret_timer_check_result:',ret_timer_check_result
                 pass##self.LogRun.write(  '\n\n*** Size of ret_timer_check_result: [ %d ]:\n ***'%len(ret_timer_check_result) )
                 ...
                 for key,val in ret_timer_check_result.items():
                    ### ---- C.1.1.1 Read the MBox-CFlow holding information.    
                     ...
                     Dst_MBox_cur = val[2];## Get the returned Dst_MBox_cur.
                     Dst_MBox_new = val[3];## Get the returned Dst_MBox_new.
                     pass##self.LogRun.write( '\n ---##-- True_Replace, begin: Src[ %d ] - Dst_MBox_cur[ %d ] - DstMBox_new[ %d ]\n'%(Src_CFlow, Dst_MBox_cur, Dst_MBox_new) )        
                    ### ---- And replace the old routing-path with the new selected routing-path for a Src_ClientFlow.
                     self.Replace_the_selected_DstMBox_and_Path_for_a_SrcCFlow( Src_CFlow, Dst_MBox_cur, Dst_MBox_new, pathID_old, pathID_new );
                ...
                ###### Throughput = Get_objVal_of_configurations_in_whole_system();
                ###### self.LogRun.write(  '\n***Finish True_Replace for [ %d ] Src-CFlows, -Throughput[ %s ],  -TS [ %s ]. ****\n\n'%(len(ret_timer_check_result),Throughput, current_ts) )
    ...
    def Initialization(self):
        ...
        #global self.PathSet_selected     ##modify by gzq
        #global self.PathSet_cand            ##modify by gzq
        print "self.TDSet"+" "+str(self.TDSet)+"\n"
	#self.f1.write("91 \n")
	    ...
	    #self.f1.write("92 \n")
            ...
            print "bool_whether_tarMBox_feasible"+" "+str(bool_whether_tarMBox_feasible)
            print  "\n"
	    #self.f1.write("93 \n")
            if (1==bool_whether_tarMBox_feasible):
                ...
                print "self.PathSet_cand"+" "+str(self.PathSet_cand)
                print "\n"
    ...
    def Check_whether_this_new_path_is_feasible_to_the_SrcCFlow(self,Src,Dst_new,Path_ID_new):
        ...
        #self.Log_debug.write("\t ---- Path_ID_new"+" "+str(Path_ID_new))
        print "Path_ID_new" +" "+str(Path_ID_new)+"\n"
        if Path_ID_new<0:
            return 0
    	#self.f1.write("333333333333333 \n")
        ...
        #self.f1.write("1111111111111 \n")
        ## --- 2. count all the currently-accumulative-assigned TrafficRate(TR) in each traversed arc.
        for (u,v) in list_arcs_in_this_path:
            ## --- 2.1 Calculate the summed TR in (u,v).
            ...
            ## --- 2.2 compare the available TR bandwidth in each arc with the demanding TR of this Src_CFlow.
	    ...
	    #self.f1.write(str(v)+"\n")
	    #self.f1.flush()
	    iii = self.arg.getcurrentcapacity(u,v)
	    #self.f1.write(str(iii)+"\n")
	    
            available_TR_in_uv = self.arg.getcurrentcapacity(u,v) - summed_TR_in_uv;
            print "available_TR_in_uv"+" "+str(available_TR_in_uv)+"\n"
            demanding_TR_of_SrcCFlow = self.TDSet[Src];
            print "demanding_TR_of_SrcCFlow"+" "+str(demanding_TR_of_SrcCFlow)+"\n"
            print "available_TR_in_uv"+" "+str(available_TR_in_uv)+"\n"
	    #self.f1.write("9999999999\n")
            if ( demanding_TR_of_SrcCFlow > available_TR_in_uv ):
		#self.f1.write("8888888 \n")
