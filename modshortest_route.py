...
class Shortest_Route(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.Log_debug.write("init complete"+"\n")
        ...
        self.Log_debug.write("link_to_port"+"  "+str(self.link_to_port)+"\n")
        self.Log_debug.write("access_table"+"  "+str(self.access_table)+"\n")
        self.Log_debug.write("access_ports"+"  "+str(self.access_ports)+"\n")
    ...
    def add_flow(self, dp, p, match, actions, idle_timeout, hard_timeout):
        ofproto = dp.ofproto
    ...
    def install_flow(self, path, flow_info, priority, buffer_id, data):
        '''
        ...
        if len(path) > 1:
            ...
            self.add_flow(datapath_first,
                          priority, match, actions, idle_timeout=600, hard_timeout=600)
            ...
            self.add_flow(
                datapath, priority, match, actions, idle_timeout=600, hard_timeout=600)
    ...
    def _packet_in_handler(self, ev):
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            ...
            ip_dst = ip_pkt.dst
            path = []
            self.Log_debug.write("ip_src"+"   "+ip_src+"\n")
		...
		self.Log_debug.write("path_list  "+" "+str(path_list)+"\n")
                ...
                self.install_flow(path, flow_info, 10, msg.buffer_id, msg.data)        
                             
           #####################################################################################################
            else:
	        self.Log_debug.write("location \n")
                result = None
                src_sw = None
                dst_sw = None
                ...
                if result:
                    ...
                    self.logger.info(
                      " PATH[%s --> %s]:%s\n" % (ip_src, ip_dst, path))
		    flow_info = (eth_type, ip_src, ip_dst, in_port)
                    self.install_flow(path, flow_info,1,msg.buffer_id, msg.data)
                else:
